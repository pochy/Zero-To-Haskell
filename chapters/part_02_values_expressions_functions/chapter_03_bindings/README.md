...
- `値`: この章の判断で使う語。`値` が何を許し、何を防ぐかに注目します。
- `束縛`: この章の判断で使う語。`束縛` が何を許し、何を防ぐかに注目します。
- `不変性`: この章の判断で使う語。`不変性` が何を許し、何を防ぐかに注目します。
- `式`: この章の判断で使う語。`式` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、値は書き換える箱ではなく名前に束縛された意味だと捉えるための制約を確認します。
