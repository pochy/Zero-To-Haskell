...
この章の目的は、**値は書き換える箱ではなく名前に束縛された意味だと捉える** ことです。

Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `値`: 状態更新ではなく式の変換としてコードを組み立てるための語です。
- `束縛`: 状態更新ではなく式の変換としてコードを組み立てるための語です。
- `不変性`: 状態更新ではなく式の変換としてコードを組み立てるための語です。
- `式`: 状態更新ではなく式の変換としてコードを組み立てるための語です。
...
この章では、`値` を中心に、目の前のコードを「何を実行するか」ではなく「どんな値を作り、どんな型で制約し、どんな計算として合成するか」という観点で読みます。
...
この章で扱う `値` は、単体のテクニックではなく、この思想をコード上の判断に落とすための入口です。関数名や演算子を覚えるだけでなく、その概念がどの不安を減らし、どの設計上の責任を明示しているのかを読んでください。
