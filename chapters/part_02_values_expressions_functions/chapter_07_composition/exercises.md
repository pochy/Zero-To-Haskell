# 第7章 Exercises: 関数合成とポイントフリースタイル

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`小さな変換を合成して読みやすい処理の流れを作る` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「小さな変換を合成して読みやすい処理の流れを作る」を、次の形式で短く説明してください。
