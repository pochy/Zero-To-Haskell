...
- `($)`: この章の判断で使う語。`($)` が何を許し、何を防ぐかに注目します。
- `ポイントフリー`: この章の判断で使う語。`ポイントフリー` が何を許し、何を防ぐかに注目します。
- `パイプライン`: この章の判断で使う語。`パイプライン` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、小さな変換を合成して読みやすい処理の流れを作るための制約を確認します。
