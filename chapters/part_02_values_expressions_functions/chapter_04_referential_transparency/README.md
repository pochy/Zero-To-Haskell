...
- `純粋関数`: この章の判断で使う語。`純粋関数` が何を許し、何を防ぐかに注目します。
- `等式推論`: この章の判断で使う語。`等式推論` が何を許し、何を防ぐかに注目します。
- `副作用`: この章の判断で使う語。`副作用` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、式を同じ値で置き換えても意味が変わらない感覚を身につけるための制約を確認します。
