...
`examples/Main.hs` を一箇所だけ変更し、`関数を値として渡し、処理の形を抽象化する` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「関数を値として渡し、処理の形を抽象化する」を、次の形式で短く説明してください。
