# 第39章 Exercises: DSL設計

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`小さな言語を型と関数で設計する` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「小さな言語を型と関数で設計する」を、次の形式で短く説明してください。
...
```text
□ Embedded DSL を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
