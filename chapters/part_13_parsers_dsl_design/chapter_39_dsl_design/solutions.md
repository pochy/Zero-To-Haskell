# 第39章 Solution Guide: DSL設計

## 観察の答え方

この章の例は、`Embedded DSL` を使って **小さな言語を型と関数で設計する** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
