...
この章の目的は、**小さなパーサーを合成して言語を読む** ことです。

Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `Parser`: 小さな部品を合成して入力言語やドメイン語彙を作るための語です。
- `parsec`: 小さな部品を合成して入力言語やドメイン語彙を作るための語です。
- `megaparsec`: 小さな部品を合成して入力言語やドメイン語彙を作るための語です。
- `Alternative`: 小さな部品を合成して入力言語やドメイン語彙を作るための語です。
...
この章では、`Parser` を中心に、目の前のコードを「何を実行するか」ではなく「どんな値を作り、どんな型で制約し、どんな計算として合成するか」という観点で読みます。
...
この章で扱う `Parser` は、単体のテクニックではなく、この思想をコード上の判断に落とすための入口です。関数名や演算子を覚えるだけでなく、その概念がどの不安を減らし、どの設計上の責任を明示しているのかを読んでください。
