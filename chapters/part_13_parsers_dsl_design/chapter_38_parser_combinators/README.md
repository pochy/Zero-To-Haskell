...
- `parsec`: この章の判断で使う語。`parsec` が何を許し、何を防ぐかに注目します。
- `megaparsec`: この章の判断で使う語。`megaparsec` が何を許し、何を防ぐかに注目します。
- `Alternative`: この章の判断で使う語。`Alternative` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、小さなパーサーを合成して言語を読むための制約を確認します。
