# 第54章 Exercises: DBを扱う

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`DB境界とドメイン境界を分ける` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「DB境界とドメイン境界を分ける」を、次の形式で短く説明してください。
...
```text
□ persistent を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
