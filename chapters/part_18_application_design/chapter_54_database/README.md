...
- `persistent`: この章の判断で使う語。`persistent` が何を許し、何を防ぐかに注目します。
- `beam`: この章の判断で使う語。`beam` が何を許し、何を防ぐかに注目します。
- `transaction`: この章の判断で使う語。`transaction` が何を許し、何を防ぐかに注目します。
- `connection pool`: この章の判断で使う語。`connection pool` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、DB境界とドメイン境界を分けるための制約を確認します。
