# 第52章 Solution Guide: CLIアプリを作る

## 観察の答え方

この章の例は、`optparse-applicative` を使って **純粋ロジックを持つCLIを作る** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
...
```text
□ optparse-applicative を使う理由を説明している
