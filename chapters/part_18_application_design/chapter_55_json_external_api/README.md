...
- `aeson`: この章の判断で使う語。`aeson` が何を許し、何を防ぐかに注目します。
- `FromJSON`: この章の判断で使う語。`FromJSON` が何を許し、何を防ぐかに注目します。
- `ToJSON`: この章の判断で使う語。`ToJSON` が何を許し、何を防ぐかに注目します。
- `newtype wrapper`: この章の判断で使う語。`newtype wrapper` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、外部表現と内部モデルを分離するための制約を確認します。
