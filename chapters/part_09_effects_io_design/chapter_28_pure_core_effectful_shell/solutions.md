# 第28章 Solution Guide: Pure Core / Effectful Shell

## 観察の答え方

この章の例は、`Pure Core` を使って **純粋な中心と副作用の薄い外側に分ける** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
