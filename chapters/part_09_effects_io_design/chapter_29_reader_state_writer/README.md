...
- `Reader`: この章の判断で使う語。`Reader` が何を許し、何を防ぐかに注目します。
- `State`: この章の判断で使う語。`State` が何を許し、何を防ぐかに注目します。
- `Writer`: この章の判断で使う語。`Writer` が何を許し、何を防ぐかに注目します。
- `環境`: この章の判断で使う語。`環境` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、環境・状態・ログを計算の文脈として扱うための制約を確認します。
