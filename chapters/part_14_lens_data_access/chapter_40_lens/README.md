...
- `Lens`: この章の判断で使う語。`Lens` が何を許し、何を防ぐかに注目します。
- `Prism`: この章の判断で使う語。`Prism` が何を許し、何を防ぐかに注目します。
- `Traversal`: この章の判断で使う語。`Traversal` が何を許し、何を防ぐかに注目します。
- `Optics`: この章の判断で使う語。`Optics` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、不変データの一部を合成可能に扱うための制約を確認します。
