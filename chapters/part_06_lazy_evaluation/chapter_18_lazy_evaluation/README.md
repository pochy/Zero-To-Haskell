...
- `非正格`: この章の判断で使う語。`非正格` が何を許し、何を防ぐかに注目します。
- `サンク`: この章の判断で使う語。`サンク` が何を許し、何を防ぐかに注目します。
- `無限リスト`: この章の判断で使う語。`無限リスト` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、必要になるまで評価しないことで可能になる表現を理解するための制約を確認します。
