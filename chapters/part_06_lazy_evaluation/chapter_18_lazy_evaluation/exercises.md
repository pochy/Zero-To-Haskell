# 第18章 Exercises: 遅延評価とは何か

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`必要になるまで評価しないことで可能になる表現を理解する` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「必要になるまで評価しないことで可能になる表現を理解する」を、次の形式で短く説明してください。
