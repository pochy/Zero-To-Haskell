...
- `スペースリーク`: この章の判断で使う語。`スペースリーク` が何を許し、何を防ぐかに注目します。
- `seq`: この章の判断で使う語。`seq` が何を許し、何を防ぐかに注目します。
- `BangPatterns`: この章の判断で使う語。`BangPatterns` が何を許し、何を防ぐかに注目します。
- `foldl'`: この章の判断で使う語。`foldl'` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、遅延と正格性のトレードオフを知るための制約を確認します。
