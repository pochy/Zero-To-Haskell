...
`examples/Main.hs` を一箇所だけ変更し、`Web UI向けに単純化された純粋関数型を比較する` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「Web UI向けに単純化された純粋関数型を比較する」を、次の形式で短く説明してください。
