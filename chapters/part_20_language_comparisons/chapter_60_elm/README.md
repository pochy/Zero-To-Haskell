...
この章の目的は、**Web UI向けに単純化された純粋関数型を比較する** ことです。Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `Elm`: この章で設計判断に使う中心語です。
- `TEA`: この章で設計判断に使う中心語です。
- `副作用分離`: この章で設計判断に使う中心語です。
- `Web UI`: この章で設計判断に使う中心語です。
...
この章の中心は `Elm` です。Haskellでは、便利だからそう書くのではなく、意味を狭めるために型や抽象を使います。意味を狭めると、呼び出し側が間違えにくくなり、コンパイラが設計の一部を検査できます。
