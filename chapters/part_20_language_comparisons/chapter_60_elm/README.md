...
- `Elm`: この章の判断で使う語。`Elm` が何を許し、何を防ぐかに注目します。
- `TEA`: この章の判断で使う語。`TEA` が何を許し、何を防ぐかに注目します。
- `副作用分離`: この章の判断で使う語。`副作用分離` が何を許し、何を防ぐかに注目します。
- `Web UI`: この章の判断で使う語。`Web UI` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、Web UI向けに単純化された純粋関数型を比較するための制約を確認します。
