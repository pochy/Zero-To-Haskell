...
- `依存型`: この章の判断で使う語。`依存型` が何を許し、何を防ぐかに注目します。
- `証明`: この章の判断で使う語。`証明` が何を許し、何を防ぐかに注目します。
- `全域性`: この章の判断で使う語。`全域性` が何を許し、何を防ぐかに注目します。
- `型駆動`: この章の判断で使う語。`型駆動` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、依存型と証明の世界との境界を知るための制約を確認します。
