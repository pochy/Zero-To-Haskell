...
この章の目的は、**Haskell的発想をTypeScriptで使う限界を知る** ことです。

Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `Effect`: 他言語との違いからHaskellの設計判断を相対化するための語です。
- `fp-ts`: 他言語との違いからHaskellの設計判断を相対化するための語です。
- `Option`: 他言語との違いからHaskellの設計判断を相対化するための語です。
- `Task`: 他言語との違いからHaskellの設計判断を相対化するための語です。
...
この章では、`Effect` を中心に、目の前のコードを「何を実行するか」ではなく「どんな値を作り、どんな型で制約し、どんな計算として合成するか」という観点で読みます。
...
この章で扱う `Effect` は、単体のテクニックではなく、この思想をコード上の判断に落とすための入口です。関数名や演算子を覚えるだけでなく、その概念がどの不安を減らし、どの設計上の責任を明示しているのかを読んでください。
