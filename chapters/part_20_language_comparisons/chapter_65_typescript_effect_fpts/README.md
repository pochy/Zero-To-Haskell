...
- `Effect`: この章の判断で使う語。`Effect` が何を許し、何を防ぐかに注目します。
- `fp-ts`: この章の判断で使う語。`fp-ts` が何を許し、何を防ぐかに注目します。
- `Option`: この章の判断で使う語。`Option` が何を許し、何を防ぐかに注目します。
- `Task`: この章の判断で使う語。`Task` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、Haskell的発想をTypeScriptで使う限界を知るための制約を確認します。
