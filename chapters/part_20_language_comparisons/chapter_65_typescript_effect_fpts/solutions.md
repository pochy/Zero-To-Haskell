# 第65章 Solution Guide: TypeScript + Effect / fp-ts

## 観察の答え方

この章の例は、`Effect` を使って **Haskell的発想をTypeScriptで使う限界を知る** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
