...
- `正格評価`: この章の判断で使う語。`正格評価` が何を許し、何を防ぐかに注目します。
- `型クラス`: この章の判断で使う語。`型クラス` が何を許し、何を防ぐかに注目します。
- `JavaScript`: この章の判断で使う語。`JavaScript` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、Haskell的設計をJavaScript圏で使う選択肢を比較するための制約を確認します。
