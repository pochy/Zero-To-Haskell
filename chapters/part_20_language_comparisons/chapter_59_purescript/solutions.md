# 第59章 Solution Guide: PureScript

## 観察の答え方

この章の例は、`PureScript` を使って **Haskell的設計をJavaScript圏で使う選択肢を比較する** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
