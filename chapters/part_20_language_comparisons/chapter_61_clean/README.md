...
- `Clean`: この章の判断で使う語。`Clean` が何を許し、何を防ぐかに注目します。
- `一意型`: この章の判断で使う語。`一意型` が何を許し、何を防ぐかに注目します。
- `遅延評価`: 値が必要になるまで評価しない評価戦略。無限リストや制御構造の表現力につながる。
- `純粋`: この章の判断で使う語。`純粋` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、Haskellに近い遅延純粋言語を比較するための制約を確認します。
