...
この章の目的は、**エラーを文字列ではなくドメイン型にする** ことです。

Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `DomainError`: この章の判断で使う語。`DomainError` が何を許し、何を防ぐかに注目します。
- `Either`: 成功値か失敗理由のどちらかを表す型。例外を隠さず設計に出す。
- `ExceptT`: この章の判断で使う語。`ExceptT` が何を許し、何を防ぐかに注目します。
- `ValidationError`: この章の判断で使う語。`ValidationError` が何を許し、何を防ぐかに注目します。
...
この章では、`DomainError` を中心に、目の前のコードを「何を実行するか」ではなく「どんな値を作り、どんな型で制約し、どんな計算として合成するか」という観点で読みます。
...
- `DomainError` は、この例でどの間違いを防いでいるか。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、エラーを文字列ではなくドメイン型にするための制約を確認します。
