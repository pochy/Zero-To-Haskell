# 第32章 Exercises: エラーを型で設計する

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`エラーを文字列ではなくドメイン型にする` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「エラーを文字列ではなくドメイン型にする」を、次の形式で短く説明してください。
...
```text
□ DomainError を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
