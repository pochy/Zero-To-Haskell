# 第32章 Solution Guide: エラーを型で設計する

## 観察の答え方

この章の例は、`DomainError` を使って **エラーを文字列ではなくドメイン型にする** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
