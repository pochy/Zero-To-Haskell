...
- `head`: この章の判断で使う語。`head` が何を許し、何を防ぐかに注目します。
- `fromJust`: この章の判断で使う語。`fromJust` が何を許し、何を防ぐかに注目します。
- `undefined`: この章の判断で使う語。`undefined` が何を許し、何を防ぐかに注目します。
- `bottom`: この章の判断で使う語。`bottom` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、実行時に壊れる関数を型で置き換えるための制約を確認します。
