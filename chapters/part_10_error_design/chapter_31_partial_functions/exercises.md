# 第31章 Exercises: partial function を避ける

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`実行時に壊れる関数を型で置き換える` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「実行時に壊れる関数を型で置き換える」を、次の形式で短く説明してください。
