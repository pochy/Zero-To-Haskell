...
- `Left`: この章の判断で使う語。`Left` が何を許し、何を防ぐかに注目します。
- `Right`: この章の判断で使う語。`Right` が何を許し、何を防ぐかに注目します。
- `エラー型`: この章の判断で使う語。`エラー型` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、失敗理由を型として扱うための制約を確認します。
