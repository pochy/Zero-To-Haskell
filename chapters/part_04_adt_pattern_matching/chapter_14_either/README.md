...
この章の目的は、**失敗理由を型として扱う** ことです。Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `Either`: この章で設計判断に使う中心語です。
- `Left`: この章で設計判断に使う中心語です。
- `Right`: この章で設計判断に使う中心語です。
- `エラー型`: この章で設計判断に使う中心語です。
...
この章の中心は `Either` です。Haskellでは、便利だからそう書くのではなく、意味を狭めるために型や抽象を使います。意味を狭めると、呼び出し側が間違えにくくなり、コンパイラが設計の一部を検査できます。
