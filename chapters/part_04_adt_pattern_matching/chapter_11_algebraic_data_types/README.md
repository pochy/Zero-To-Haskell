...
- `data`: この章の判断で使う語。`data` が何を許し、何を防ぐかに注目します。
- `直和型`: この章の判断で使う語。`直和型` が何を許し、何を防ぐかに注目します。
- `直積型`: この章の判断で使う語。`直積型` が何を許し、何を防ぐかに注目します。
- `コンストラクタ`: この章の判断で使う語。`コンストラクタ` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、不正な状態を型で表現できなくするための制約を確認します。
