# 第11章 Exercises: 代数的データ型

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`不正な状態を型で表現できなくする` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「不正な状態を型で表現できなくする」を、次の形式で短く説明してください。
