...
- `Just`: この章の判断で使う語。`Just` が何を許し、何を防ぐかに注目します。
- `Nothing`: この章の判断で使う語。`Nothing` が何を許し、何を防ぐかに注目します。
- `nullを避ける`: この章の判断で使う語。`nullを避ける` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、値がない可能性を型に出すための制約を確認します。
