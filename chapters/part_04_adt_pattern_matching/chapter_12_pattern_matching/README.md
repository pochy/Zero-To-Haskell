...
- `パターンマッチ`: この章の判断で使う語。`パターンマッチ` が何を許し、何を防ぐかに注目します。
- `case`: この章の判断で使う語。`case` が何を許し、何を防ぐかに注目します。
- `網羅性`: この章の判断で使う語。`網羅性` が何を許し、何を防ぐかに注目します。
- `分解`: この章の判断で使う語。`分解` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型の形に沿って安全に値を分解するための制約を確認します。
