...
- `TVar`: この章の判断で使う語。`TVar` が何を許し、何を防ぐかに注目します。
- `atomically`: この章の判断で使う語。`atomically` が何を許し、何を防ぐかに注目します。
- `retry`: この章の判断で使う語。`retry` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、共有状態をトランザクションとして扱うための制約を確認します。
