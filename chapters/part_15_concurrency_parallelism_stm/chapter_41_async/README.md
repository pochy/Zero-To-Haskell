...
- `forkIO`: この章の判断で使う語。`forkIO` が何を許し、何を防ぐかに注目します。
- `async`: この章の判断で使う語。`async` が何を許し、何を防ぐかに注目します。
- `wait`: この章の判断で使う語。`wait` が何を許し、何を防ぐかに注目します。
- `concurrently`: この章の判断で使う語。`concurrently` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、並行処理を構造化して扱うための制約を確認します。
