# 第44章 Exercises: プロファイリング

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`性能問題を推測ではなく測定する` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「性能問題を推測ではなく測定する」を、次の形式で短く説明してください。
...
```text
□ time profiling を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
