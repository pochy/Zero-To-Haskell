# 第44章 Solution Guide: プロファイリング

## 観察の答え方

この章の例は、`time profiling` を使って **性能問題を推測ではなく測定する** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
