...
- `deriving`: この章の判断で使う語。`deriving` が何を許し、何を防ぐかに注目します。
- `Generic`: この章の判断で使う語。`Generic` が何を許し、何を防ぐかに注目します。
- `DerivingVia`: この章の判断で使う語。`DerivingVia` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、導出を便利さだけでなく意味として扱うための制約を確認します。
