# 第49章 Solution Guide: DataKinds / TypeFamilies / Type-level programming

## 観察の答え方

この章の例は、`DataKinds` を使って **型レベル計算の使いどころを判断する** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
