...
- `DataKinds`: この章の判断で使う語。`DataKinds` が何を許し、何を防ぐかに注目します。
- `TypeFamilies`: この章の判断で使う語。`TypeFamilies` が何を許し、何を防ぐかに注目します。
- `TypeLits`: この章の判断で使う語。`TypeLits` が何を許し、何を防ぐかに注目します。
- `型レベル`: この章の判断で使う語。`型レベル` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型レベル計算の使いどころを判断するための制約を確認します。
