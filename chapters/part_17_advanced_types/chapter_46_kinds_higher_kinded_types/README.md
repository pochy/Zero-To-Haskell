...
- `kind`: この章の判断で使う語。`kind` が何を許し、何を防ぐかに注目します。
- `Type`: この章の判断で使う語。`Type` が何を許し、何を防ぐかに注目します。
- `* -> *`: この章の判断で使う語。`* -> *` が何を許し、何を防ぐかに注目します。
- `高カインド型`: この章の判断で使う語。`高カインド型` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型の型を読み、高カインド型を理解するための制約を確認します。
