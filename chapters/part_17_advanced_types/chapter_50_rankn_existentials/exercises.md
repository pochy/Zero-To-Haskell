# 第50章 Exercises: RankNTypes / Existential Types

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`高度な多相性で抽象境界を作る` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「高度な多相性で抽象境界を作る」を、次の形式で短く説明してください。
...
```text
□ RankNTypes を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
