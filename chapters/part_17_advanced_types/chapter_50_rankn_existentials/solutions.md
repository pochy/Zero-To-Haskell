# 第50章 Solution Guide: RankNTypes / Existential Types

## 観察の答え方

この章の例は、`RankNTypes` を使って **高度な多相性で抽象境界を作る** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
