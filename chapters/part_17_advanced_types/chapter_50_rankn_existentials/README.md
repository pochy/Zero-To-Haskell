...
- `RankNTypes`: この章の判断で使う語。`RankNTypes` が何を許し、何を防ぐかに注目します。
- `forall`: この章の判断で使う語。`forall` が何を許し、何を防ぐかに注目します。
- `existential`: この章の判断で使う語。`existential` が何を許し、何を防ぐかに注目します。
- `抽象化`: この章の判断で使う語。`抽象化` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、高度な多相性で抽象境界を作るための制約を確認します。
