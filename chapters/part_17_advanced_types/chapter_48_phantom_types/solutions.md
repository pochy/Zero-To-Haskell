# 第48章 Solution Guide: Phantom Types

## 観察の答え方

この章の例は、`Phantom type` を使って **値にない状態や権限を型に持たせる** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
