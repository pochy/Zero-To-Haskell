# 第48章 Exercises: Phantom Types

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`値にない状態や権限を型に持たせる` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「値にない状態や権限を型に持たせる」を、次の形式で短く説明してください。
...
```text
□ Phantom type を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
