...
この章の目的は、**独立した文脈付き計算を組み合わせる** ことです。
...
- `pure`: 文脈付き計算をどの程度の依存関係で合成するか判断するための語です。
- `<*>`: 文脈付き計算をどの程度の依存関係で合成するか判断するための語です。
- `独立性`: 文脈付き計算をどの程度の依存関係で合成するか判断するための語です。
...
この章では、`Applicative` を中心に、目の前のコードを「何を実行するか」ではなく「どんな値を作り、どんな型で制約し、どんな計算として合成するか」という観点で読みます。
...
この章で扱う `Applicative` は、単体のテクニックではなく、この思想をコード上の判断に落とすための入口です。関数名や演算子を覚えるだけでなく、その概念がどの不安を減らし、どの設計上の責任を明示しているのかを読んでください。
...
- `Applicative` は、この例でどの間違いを防いでいるか。
