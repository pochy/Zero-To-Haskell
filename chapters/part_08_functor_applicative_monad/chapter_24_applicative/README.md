...
- `pure`: この章の判断で使う語。`pure` が何を許し、何を防ぐかに注目します。
- `<*>`: この章の判断で使う語。`<*>` が何を許し、何を防ぐかに注目します。
- `独立性`: この章の判断で使う語。`独立性` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、独立した文脈付き計算を組み合わせるための制約を確認します。
