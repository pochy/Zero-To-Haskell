...
- `STM`: この章の判断で使う語。`STM` が何を許し、何を防ぐかに注目します。
- `benchmark`: この章の判断で使う語。`benchmark` が何を許し、何を防ぐかに注目します。
- `package`: この章の判断で使う語。`package` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型安全なアーキテクチャを設計・実装・説明するための制約を確認します。
