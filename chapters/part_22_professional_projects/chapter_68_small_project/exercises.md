# 第68章 Exercises: 小規模演習

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`安全な小さなライブラリやCLIを完成させる` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「安全な小さなライブラリやCLIを完成させる」を、次の形式で短く説明してください。
...
```text
□ safe list を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
