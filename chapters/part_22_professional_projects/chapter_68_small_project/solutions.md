# 第68章 Solution Guide: 小規模演習

## 観察の答え方

この章の例は、`safe list` を使って **安全な小さなライブラリやCLIを完成させる** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
