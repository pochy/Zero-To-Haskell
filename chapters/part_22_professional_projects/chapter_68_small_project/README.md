...
この章の目的は、**安全な小さなライブラリやCLIを完成させる** ことです。

Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `safe list`: 型、効果、性能、運用をまとめて設計するための語です。
- `CLI`: 型、効果、性能、運用をまとめて設計するための語です。
- `Either`: 成功値か失敗理由のどちらかを表す型。例外を隠さず設計に出す。
- `テスト`: 型、効果、性能、運用をまとめて設計するための語です。
...
この章では、`safe list` を中心に、目の前のコードを「何を実行するか」ではなく「どんな値を作り、どんな型で制約し、どんな計算として合成するか」という観点で読みます。
...
この章で扱う `safe list` は、単体のテクニックではなく、この思想をコード上の判断に落とすための入口です。関数名や演算子を覚えるだけでなく、その概念がどの不安を減らし、どの設計上の責任を明示しているのかを読んでください。
