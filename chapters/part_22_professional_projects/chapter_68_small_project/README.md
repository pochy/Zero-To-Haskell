...
- `safe list`: この章の判断で使う語。`safe list` が何を許し、何を防ぐかに注目します。
- `CLI`: この章の判断で使う語。`CLI` が何を許し、何を防ぐかに注目します。
- `Either`: 成功値か失敗理由のどちらかを表す型。例外を隠さず設計に出す。
- `テスト`: この章の判断で使う語。`テスト` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、安全な小さなライブラリやCLIを完成させるための制約を確認します。
