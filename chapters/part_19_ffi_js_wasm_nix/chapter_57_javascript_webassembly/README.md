...
- `JavaScript backend`: この章の判断で使う語。`JavaScript backend` が何を許し、何を防ぐかに注目します。
- `WebAssembly backend`: この章の判断で使う語。`WebAssembly backend` が何を許し、何を防ぐかに注目します。
- `PureScript`: この章の判断で使う語。`PureScript` が何を許し、何を防ぐかに注目します。
- `frontend`: この章の判断で使う語。`frontend` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、HaskellをWeb実行環境へ接続する選択肢を知るための制約を確認します。
