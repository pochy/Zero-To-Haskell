...
この章の目的は、**HaskellをWeb実行環境へ接続する選択肢を知る** ことです。Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `JavaScript backend`: この章で設計判断に使う中心語です。
- `WebAssembly backend`: この章で設計判断に使う中心語です。
- `PureScript`: この章で設計判断に使う中心語です。
- `frontend`: この章で設計判断に使う中心語です。
...
この章の中心は `JavaScript backend` です。Haskellでは、便利だからそう書くのではなく、意味を狭めるために型や抽象を使います。意味を狭めると、呼び出し側が間違えにくくなり、コンパイラが設計の一部を検査できます。
