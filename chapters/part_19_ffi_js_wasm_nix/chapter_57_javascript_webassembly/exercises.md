# 第57章 Exercises: JavaScript / WebAssembly backend

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`HaskellをWeb実行環境へ接続する選択肢を知る` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「HaskellをWeb実行環境へ接続する選択肢を知る」を、次の形式で短く説明してください。
...
```text
□ JavaScript backend を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
