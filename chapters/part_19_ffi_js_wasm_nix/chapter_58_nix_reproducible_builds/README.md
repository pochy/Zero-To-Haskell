...
- `Nix`: この章の判断で使う語。`Nix` が何を許し、何を防ぐかに注目します。
- `haskell.nix`: この章の判断で使う語。`haskell.nix` が何を許し、何を防ぐかに注目します。
- `CI`: この章の判断で使う語。`CI` が何を許し、何を防ぐかに注目します。
- `再現可能`: この章の判断で使う語。`再現可能` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、環境差分を減らすビルド設計を知るための制約を確認します。
