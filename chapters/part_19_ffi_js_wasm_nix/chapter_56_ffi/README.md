...
- `FFI`: この章の判断で使う語。`FFI` が何を許し、何を防ぐかに注目します。
- `C連携`: この章の判断で使う語。`C連携` が何を許し、何を防ぐかに注目します。
- `marshal`: この章の判断で使う語。`marshal` が何を許し、何を防ぐかに注目します。
- `境界`: この章の判断で使う語。`境界` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、外部言語との境界を安全に扱うための制約を確認します。
