...
- `純粋性`: この章の判断で使う語。`純粋性` が何を許し、何を防ぐかに注目します。
- `型で設計`: この章の判断で使う語。`型で設計` が何を許し、何を防ぐかに注目します。
- `法則`: この章の判断で使う語。`法則` が何を許し、何を防ぐかに注目します。
- `抽象`: この章の判断で使う語。`抽象` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型・純粋性・合成・法則を判断軸にするための制約を確認します。
