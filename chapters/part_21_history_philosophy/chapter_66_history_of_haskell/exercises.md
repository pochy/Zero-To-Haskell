# 第66章 Exercises: なぜHaskellは生まれたのか

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`歴史から設計判断の理由を読む` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「歴史から設計判断の理由を読む」を、次の形式で短く説明してください。
...
```text
□ Haskell 98 を、この章のコードと結びつけて説明できる
□ サンプルの型注釈を読める
□ 純粋関数とIOアクションを区別できる
