...
- `Haskell 98`: この章の判断で使う語。`Haskell 98` が何を許し、何を防ぐかに注目します。
- `Haskell 2010`: この章の判断で使う語。`Haskell 2010` が何を許し、何を防ぐかに注目します。
- `GHC`: この章の判断で使う語。`GHC` が何を許し、何を防ぐかに注目します。
- `研究`: この章の判断で使う語。`研究` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、歴史から設計判断の理由を読むための制約を確認します。
