# 第66章 Solution Guide: なぜHaskellは生まれたのか

## 観察の答え方

この章の例は、`Haskell 98` を使って **歴史から設計判断の理由を読む** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
