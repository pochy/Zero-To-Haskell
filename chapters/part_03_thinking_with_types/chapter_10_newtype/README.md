...
この章の目的は、**意味の違う値を型で分ける** ことです。Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `type`: この章で設計判断に使う中心語です。
- `newtype`: この章で設計判断に使う中心語です。
- `ドメインモデル`: この章で設計判断に使う中心語です。
- `型安全`: この章で設計判断に使う中心語です。
...
この章の中心は `type` です。Haskellでは、便利だからそう書くのではなく、意味を狭めるために型や抽象を使います。意味を狭めると、呼び出し側が間違えにくくなり、コンパイラが設計の一部を検査できます。
