# 第10章 Solution Guide: 型エイリアスと newtype

## 観察の答え方

この章の例は、`newtype` を使って **意味の違う値を型で分ける** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
