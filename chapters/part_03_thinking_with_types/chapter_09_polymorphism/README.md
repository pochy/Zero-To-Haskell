...
- `多相性`: この章の判断で使う語。`多相性` が何を許し、何を防ぐかに注目します。
- `型変数`: この章の判断で使う語。`型変数` が何を許し、何を防ぐかに注目します。
- `パラメトリック`: この章の判断で使う語。`パラメトリック` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型変数から関数ができることとできないことを読むための制約を確認します。
