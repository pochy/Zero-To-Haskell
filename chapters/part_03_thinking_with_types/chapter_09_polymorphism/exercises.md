...
`examples/Main.hs` を一箇所だけ変更し、`型変数から関数ができることとできないことを読む` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「型変数から関数ができることとできないことを読む」を、次の形式で短く説明してください。
