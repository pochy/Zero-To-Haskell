# 第8章 Exercises: 型注釈と型推論

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`型をコンパイラ向け情報ではなく設計メモとして使う` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「型をコンパイラ向け情報ではなく設計メモとして使う」を、次の形式で短く説明してください。
