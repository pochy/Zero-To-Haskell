...
- `型注釈`: この章の判断で使う語。`型注釈` が何を許し、何を防ぐかに注目します。
- `型推論`: この章の判断で使う語。`型推論` が何を許し、何を防ぐかに注目します。
- `静的型付け`: この章の判断で使う語。`静的型付け` が何を許し、何を防ぐかに注目します。
- `仕様`: この章の判断で使う語。`仕様` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型をコンパイラ向け情報ではなく設計メモとして使うための制約を確認します。
