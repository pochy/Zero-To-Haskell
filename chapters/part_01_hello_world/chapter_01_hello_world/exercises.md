# 第1章 Exercises: Hello, World! の本当の意味

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、``IO ()` を命令ではなく値として理解する` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「`IO ()` を命令ではなく値として理解する」を、次の形式で短く説明してください。
