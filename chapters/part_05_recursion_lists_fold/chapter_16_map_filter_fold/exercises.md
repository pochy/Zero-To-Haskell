# 第16章 Exercises: map / filter / fold

## 1. 最小変更で観察する

`examples/Main.hs` を一箇所だけ変更し、`リスト処理を変換・選別・畳み込みとして読む` という観点で何が変わったかを説明してください。変更前に出力を予想してから実行します。
...
「リスト処理を変換・選別・畳み込みとして読む」を、次の形式で短く説明してください。
