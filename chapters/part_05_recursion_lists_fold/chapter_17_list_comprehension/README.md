...
- `リスト内包表記`: この章の判断で使う語。`リスト内包表記` が何を許し、何を防ぐかに注目します。
- `generator`: この章の判断で使う語。`generator` が何を許し、何を防ぐかに注目します。
- `guard`: この章の判断で使う語。`guard` が何を許し、何を防ぐかに注目します。
- `宣言的`: この章の判断で使う語。`宣言的` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、宣言的にリストを生成するための制約を確認します。
