...
- `property`: この章の判断で使う語。`property` が何を許し、何を防ぐかに注目します。
- `Arbitrary`: この章の判断で使う語。`Arbitrary` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、性質をランダム生成で検査するための制約を確認します。
