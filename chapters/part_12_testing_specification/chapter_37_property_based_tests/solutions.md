# 第37章 Solution Guide: プロパティベーステスト

## 観察の答え方

この章の例は、`QuickCheck` を使って **性質をランダム生成で検査する** ことを確認するための最小コードです。出力だけを写すのではなく、どの定義が純粋で、どの定義が `IO` の境界にあるかを説明します。
