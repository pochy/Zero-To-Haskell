...
この章の目的は、**具体例で仕様の基礎を固定する** ことです。Haskellでは構文を覚えるだけでは足りません。値、型、副作用、合成、法則のどれを今扱っているのかを意識すると、短いコードでも設計の意図が読めるようになります。

## まず知るべき言葉

- `Hspec`: この章で設計判断に使う中心語です。
- `tasty`: この章で設計判断に使う中心語です。
- `unit test`: この章で設計判断に使う中心語です。
- `doctest`: この章で設計判断に使う中心語です。
...
この章の中心は `Hspec` です。Haskellでは、便利だからそう書くのではなく、意味を狭めるために型や抽象を使います。意味を狭めると、呼び出し側が間違えにくくなり、コンパイラが設計の一部を検査できます。
