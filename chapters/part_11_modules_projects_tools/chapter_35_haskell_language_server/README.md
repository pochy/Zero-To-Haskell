...
- `HLS`: この章の判断で使う語。`HLS` が何を許し、何を防ぐかに注目します。
- `LSP`: この章の判断で使う語。`LSP` が何を許し、何を防ぐかに注目します。
- `診断`: この章の判断で使う語。`診断` が何を許し、何を防ぐかに注目します。
- `補完`: この章の判断で使う語。`補完` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型を見ながら開発する環境を作るための制約を確認します。
