...
- `module`: この章の判断で使う語。`module` が何を許し、何を防ぐかに注目します。
- `import`: この章の判断で使う語。`import` が何を許し、何を防ぐかに注目します。
- `export list`: この章の判断で使う語。`export list` が何を許し、何を防ぐかに注目します。
- `抽象データ型`: この章の判断で使う語。`抽象データ型` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、公開境界で不変条件を守るための制約を確認します。
