...
- `Cabal`: この章の判断で使う語。`Cabal` が何を許し、何を防ぐかに注目します。
- `Stack`: この章の判断で使う語。`Stack` が何を許し、何を防ぐかに注目します。
- `Hackage`: この章の判断で使う語。`Hackage` が何を許し、何を防ぐかに注目します。
- `依存関係`: この章の判断で使う語。`依存関係` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、Haskellプロジェクトと依存関係を管理するための制約を確認します。
