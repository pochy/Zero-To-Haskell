...
- `Functor laws`: この章の判断で使う語。`Functor laws` が何を許し、何を防ぐかに注目します。
- `Monoid laws`: この章の判断で使う語。`Monoid laws` が何を許し、何を防ぐかに注目します。
- `law testing`: この章の判断で使う語。`law testing` が何を許し、何を防ぐかに注目します。
- `信頼`: この章の判断で使う語。`信頼` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型が合うだけでなく期待される法則を守るための制約を確認します。
