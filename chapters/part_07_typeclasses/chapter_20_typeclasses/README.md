...
- `インスタンス`: この章の判断で使う語。`インスタンス` が何を許し、何を防ぐかに注目します。
- `制約`: この章の判断で使う語。`制約` が何を許し、何を防ぐかに注目します。
- `オーバーロード`: この章の判断で使う語。`オーバーロード` が何を許し、何を防ぐかに注目します。
...
A. 小さな実験では構いません。ただし、この章では先に型を読み、型ごとの共通操作を法則付きの抽象として扱うための制約を確認します。
